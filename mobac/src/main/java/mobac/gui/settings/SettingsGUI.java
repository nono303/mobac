...
public class SettingsGUI extends JDialog {
	...
	private SettingsGUI(JFrame owner) {
		...
		loadSettings();
		addListeners();
		LOG.trace("Settings dialog preparation finished");
		...
		LOG.trace("Make settings dialog visible");
		setVisible(true);
	}
	...
	private void addDisplaySettingsPanel() {
		LOG.trace("addDisplaySettingsPanel");
		...
		unitSystemPanel.add(new JLabel(I18nUtils.localizedStringForKey("set_display_unit_system_scale_bar")),
				GBC.std());
	...
	private void addMapSourceSettingsPanel() throws URISyntaxException {
		LOG.trace("addMapSourceSettingsPanel");
		...
		osmHikingTicketUrl.addMouseListener(
				new OpenInWebbrowser(I18nUtils.localizedStringForKey("set_mapsrc_config_osmhiking_howto_url")));
	...
	private void addMapSourceManagerPanel() {
		LOG.trace("addMapSourceManagerPanel");
	...
	private void addNetworkPanel() {
		...
		panel.add(new JLabel(I18nUtils.localizedStringForKey("set_net_bandwidth_desc")),
				GBC.eol().fill(GBC.HORIZONTAL));
	...
	/**
	 * Reads the user defined settings from the gui and updates the {@link Settings}
	 * values according to the read gui settings.
	 */
	private void applySettings() {
		...
		try {
			...
			JOptionPane.showMessageDialog(null,
					String.format(I18nUtils.localizedStringForKey("set_error_saving_msg"),
	...
	private class MapPacksOnlineUpdateAction implements ActionListener {
		...
		public void actionPerformed(ActionEvent event) {
			...
			// String msg = (result) ? "Online update successfull" : "No new update
			// avilable";
			...
			// JOptionPane.showMessageDialog(SettingsGUI.this, e.getMessage(), "Mapsources
