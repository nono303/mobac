...
public class PreviewMap extends JMapViewer {
	...
	/**
	 * Pre-painted transparent tile with grid lines on it. This makes painting the
	 * grid a lot faster in difference to painting each line or rectangle if the
	 * grid zoom is much higher that the current zoom level.
	 */
	...
	public void setMapSource(MapSource newMapSource) {
		setMapSource(newMapSource, false);
	}

	public void setMapSource(MapSource newMapSource, boolean force) {
	...
	public void setIsMeasuring(Boolean val) {
		isMeasuring = val;
		MainGUI.getMainGUI().setEnableLeftPanel(!isMeasuring);
	}

	/**
	 * Updates the <code>gridSize</code> and the <code>gridTile</code>. This method
	...
	/**
	 * @return Coordinate of the point in the center of the currently displayed map
	 *         region
	 */
	...
	/**
	 * @return Coordinate of the top left corner visible regarding the current map
	 *         source (pixel)
	 */
	 ...
	 *            zoom level
	 * @param pEnd
	 *            x/y tile coordinate of the bottom right tile regarding the current
	 *            zoom level
	 * @param notifyListeners
	 */
	...
	/**
	 * Sets the rectangular selection to the absolute tile coordinates
	 * <code>pStart</code> and <code>pEnd</code> regarding the zoom-level
	 * <code>cZoom</code>.
	 *
	 * @param cZoom
	 * @param pStart
	...
	public void setSelectionByTileCoordinate(int cZoom, Point pStart, Point pEnd, boolean notifyListeners) {
		...
		// Sort x/y coordinate of points so that pNewStart < pnewEnd and limit selection
		// to map size
	 ...
	 * {@link MapEventListener#selectionChanged(MercatorPixelCoordinate, MercatorPixelCoordinate)}
	 * event.
	 */
	...
	/**
	 * Clears the in-memory tile cache and performs a repaint which causes a reload
	 * of all displayed tiles (from disk or if not present from the map source via
	 * network).
	 */
	public void refreshMap() {
		tileCache.clear();
		if (mapSource instanceof RefreshableMapSource) {
			((RefreshableMapSource) mapSource).refresh();
		}
		repaint();
	}

	public void runRuler(String action) {
		if (action == "toggle") {
			action = isMeasuring ? "stop" : "start";
		}
	...
	/**
	 * Sets a new mapSelectionController. Previous controller are disabled and
	 * removed.
	 *
