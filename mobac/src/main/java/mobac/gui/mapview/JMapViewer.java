...
public class JMapViewer extends JPanel implements TileLoaderListener {
	...
	/**
	 * Sets the displayed map pane and zoom level so that the two points (x1/y1) and
	 * (x2/y2) visible. Please note that the coordinates have to be specified
	...
	public void setMapSource(MapSource newMapSource) {
		...
		zoomSlider.setMaximum(newMapSource.getMaxZoom());
		jobDispatcher.cancelOutstandingJobs();
		if (zoom > newMapSource.getMaxZoom()) {
			setZoom(newMapSource.getMaxZoom());
		}

		if (newMapSource instanceof MapSourceInitialDisplayPosition) {
			MapSourceInitialDisplayPosition vp = (MapSourceInitialDisplayPosition) newMapSource;
			if (vp.getInitialDisplayPositionX() != 0) {
				setZoom(newMapSource.getMinZoom());
				setDisplayPosition(new Point(0, 0), vp.getInitialDisplayPositionX(), vp.getInitialDisplayPositionY(),
						newMapSource.getMinZoom());
			}
		}

		mapTileLayers.clear();
		log.info("Map layer changed to: " + newMapSource);
