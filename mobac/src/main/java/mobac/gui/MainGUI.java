...
public class MainGUI extends JFrame implements MapEventListener {
	...
	private MainGUI() {
		...
		centerRightPanel.add(layeredPane, BorderLayout.CENTER);
		centerRightPanel.add(rightPanel, BorderLayout.EAST);

		leftPanel.setMinimumSize(new Dimension(LEFT_PANEL_MIN_SIZE, 100));
		splitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, true, leftPanel, centerRightPanel);
	...
	private void createControls() {
		...
		rulerCheckBox.setVisible(true);
		rulerCheckBox.setText(I18nUtils.localizedStringForKey("map_ctrl_ruler_title"));
		rulerCheckBox.setToolTipText(I18nUtils.localizedStringForKey("map_ctrl_ruler_tooltip"));
		rulerCheckBox.setOpaque(true);
		...
		mapSourceTree.addMouseMotionListener(new MouseMotionAdapter() {
			...
			public void mouseMoved(MouseEvent e) {
				...
				jTree.setCursor(isLocationClickable
	...
	private void prepareMenuBar() {
		...
		JMenuItem addGpxTrackSelection = new JMenuItem2(
				I18nUtils.localizedStringForKey("menu_selection_selection_add_around_gpx"),
				AddGpxTrackPolygonMap.class);
		...
		JMenuItem addGpxTrackAreaSelection = new JMenuItem2(
				I18nUtils.localizedStringForKey("menu_selection_selection_add_by_gpx"),
		...
		Level[] list = new Level[]{Level.TRACE, Level.DEBUG, Level.INFO, Level.WARN, Level.ERROR, Level.OFF};
		ActionListener al = new DebugSetLogLevel();
		ch.qos.logback.classic.Logger root = (ch.qos.logback.classic.Logger) LoggerFactory
				.getLogger(org.slf4j.Logger.ROOT_LOGGER_NAME);
		Level rootLogLevel = root.getLevel();
		...
		settingsMenuItem.setIcon(
				new ImageIcon(MainGUI.class.getResource("/mobac/resources/images/menu_icons/menu_settings.png")));
		...
		showRightPanel.setIcon(
				new ImageIcon(MainGUI.class.getResource("/mobac/resources/images/menu_icons/menu_panels_right.png")));
		...
		showLeftPanel.setIcon(
				new ImageIcon(MainGUI.class.getResource("/mobac/resources/images/menu_icons/menu_panels_left.png")));

		showLeftPanel.addActionListener(new SplitPaneHideLeftComponent(splitPane));
		...
		licenses.setIcon(
				new ImageIcon(MainGUI.class.getResource("/mobac/resources/images/menu_icons/menu_licenses.png")));
	...
	public void loadExternalToolsMenu() {
		if (ExternalToolsLoader.load()) {
			if (externalToolsMenu == null) {
				...
				externalToolsMenu.setIcon(new ImageIcon(
						MainGUI.class.getResource("/mobac/resources/images/menu_icons/menu_external_tools.png")));
			...
			for (ExternalToolDef t : ExternalToolsLoader.tools) {
				...
				externalToolMenuItem.setIcon(new ImageIcon(
						MainGUI.class.getResource("/mobac/resources/images/menu_icons/menu_external_tool_item.png")));
	...
	private void updateLeftPanel() {
		...
		zoomLevelsPanel = new JCollapsiblePanel(I18nUtils.localizedStringForKey("lp_zoom_title"), new GridBagLayout());
		...
		atlasContentPanel = new JCollapsiblePanel(I18nUtils.localizedStringForKey("lp_atlas_title"),
				new GridBagLayout());
	...
	private JPanel updateMapControlsPanel() {
		...
		topControls.add(rulerCheckBox, GBC.std().insets(10, 5, 0, 0));
		topControls.add(rulerButtonClear, GBC.std().insets(10, 5, 0, 0));
	...
	public void updateBookmarksMenu() {
		...
		for (Bookmark bookmark : Settings.getInstance().placeBookmarks) {
			...
			bookmarkMenuItem.setIcon(new ImageIcon(
					MainGUI.class.getResource("/mobac/resources/images/menu_icons/menu_bookmark_item.png")));
			bookmarkMenu.add(bookmarkMenuItem);
		}
	}

	public void setEnableLeftPanel(boolean enabled) {
		zoomLevelsPanel.setVisible(enabled);
	...
	public void mapSourceChanged(MapSource newMapSource) {
		...
		// newMapSource.equals(mapSourceCombo.getSelectedItem()))
		// return;
				...
				try {
					...
					JOptionPane.showMessageDialog(null,
							I18nUtils.localizedStringForKey("msg_map_source_initialization_failed",
									selectedMapSource.toString(), e1.getLocalizedMessage()),
							I18nUtils.localizedStringForKey("Error"), JOptionPane.ERROR_MESSAGE);
				}
			}
	...
	/**
	 * Saves the window position and size when window is moved or resized. This is
	 * necessary because of the maximized state. If a window is maximized it is
	 * impossible to retrieve the window size & position of the non-maximized window
	 * - therefore we have to collect the information every time they change.
	 */
