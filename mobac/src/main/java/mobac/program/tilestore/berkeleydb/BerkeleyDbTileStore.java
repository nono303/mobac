...
public class BerkeleyDbTileStore extends TileStore {
	...
	protected void acquireTileStoreLock() throws TileStoreException {
		try {
			...
			if (tileStoreLock == null) {
				throw new TileStoreException(
						"Unable to obtain tile store lock - another instance of Mobile Atlas Creator is running!");
			}
	...
	/**
	 * Returns the directory used for storing the tile database of the
	 * {@link MapSource} specified by <code>mapSource</code>
	 *
	 * @param mapSource
	 * @return
	 */
	...
	protected class TileDatabase implements AutoCloseable {
		...
		public TileDatabase(String mapSourceName, File databaseDirectory) throws IOException, DatabaseException {
		...
		public BufferedImage getCacheCoverage(int zoom, Point tileNumMin, Point tileNumMax)
			...
			// We are loading the coverage of the selected area column by column which is
			// much faster than loading the
