...
public class TileDownLoader {
    ...
    public static byte[] downloadTileAndUpdateStore(int x, int y, int zoom, HttpMapSource mapSource,
        ...
        Optional<Date> lastMod = Arrays.stream(result.headers).filter(h -> "Last-Modified".equalsIgnoreCase(h.getName()))
                .map(h -> DateUtils.parseDate(h.getValue())).filter(Objects::nonNull).findFirst();

        Optional<Date> exp = Arrays.stream(result.headers).filter(h -> "Expires".equalsIgnoreCase(h.getName()))
                .map(h -> DateUtils.parseDate(h.getValue())).filter(Objects::nonNull).findFirst();

        long timeLastModified = 0;
        long timeExpires = 0;
        String eTag = Arrays.stream(result.headers).filter(h -> "ETag".equalsIgnoreCase(h.getName()))
                .map(NameValuePair::getValue).findFirst().orElse(null);

        if (lastMod.isPresent()) {
            timeLastModified = lastMod.get().getTime();
        }
        if (exp.isPresent()) {
            timeExpires = exp.get().getTime();
        }


        Utilities.checkForInterruption();
        TileImageType imageType = ImageFormatDetector.getImageType(result.data);
        if (imageType == null) {
            throw new UnrecoverableDownloadException("The returned image is of unknown format");
        }
        if (useTileStore) {
            TileStore.getInstance().putTileData(result.data, x, y, zoom, mapSource, timeLastModified, timeExpires, eTag);
        }
        Utilities.checkForInterruption();
        return result.data;
    }
        ...
        if (contentType != null) {
            ...
            if (!contentType.startsWith("image/")) {
                if (log.isTraceEnabled() && contentType.startsWith("text/")) {
                    String msg = "Content (" + contentType + "): " + new String(data, StandardCharsets.UTF_8);
                    log.trace(msg);
                }
                throw new DownloadFailedException(conn, data);
            }
    ...
    protected static void checkContentType(HttpResult httpResult)
            throws DownloadFailedException, IOException {
        ...
        if (contentType != null) {
            ...
            if (!contentType.startsWith("image/")) {
                if (log.isTraceEnabled() && contentType.startsWith("text/")) {
                    String msg = "Content (" + contentType + "): " + new String(httpResult.data, StandardCharsets.UTF_8);
                    log.trace(msg);
                }
                throw new RuntimeException("Server returned not an image: " + contentType);
            }
        }
    }
    ...
    private static class HttpResult {
        ...
        public HttpResult(int statusCode, Header[] headers, String contentType, byte[] data) {
