...
public class TrekBuddy extends AtlasCreator {
	 ...
	 * implementation needs much more CPU power as each original tile is loaded at
	 * least once and each generated tile has to be saved.
	 *
	...
	public void createAtlasTbaFile(String name) {
		...
		try (FileWriter fw = new FileWriter(crtba)) {
			fw.write("Atlas 1.0\r\n");
		} catch (IOException e) {
			log.error("", e);
		}
	...
	private class FileTileWriter implements MapTileWriter {
		...
		public FileTileWriter() throws IOException {
			...
			try {
				setFileWriter = new BufferedWriter(
						new OutputStreamWriter(new FileOutputStream(setFile), TEXT_FILE_CHARSET));
			} catch (IOException e) {
				log.error("", e);
			}
		}
