...
public class MapTileBuilder {
	...
	private static final Logger log = LoggerFactory.getLogger(MapTileBuilder.class);
	...
	private final TileImageDataWriterBuilder tileImageDataWriterBuilder;
	...
	/**
	 * A simple local cache holding the last 10 loaded original tiles. If the custom
	 * tile size is smaller than 256x256 the efficiency of this cache is very high
	 * (~ 75% hit rate).
	 */
	 ...
	 *            affects the tile size at the left and bottom border of the map. If
	 *            <code>true</code> those tile will have the size of the remaining
	 *            image data available size (they can be smaller than the size
	 *            specified). If <code>false</code> the tile size will be as
	 *            specified by the map's {@link TileImageParameters}.
	 */
	public MapTileBuilder(AtlasCreator atlasCreator, MapTileWriter mapTileWriter, boolean useRealTileSize) {
		this(atlasCreator, atlasCreator.getParameters().getFormat().getDataWriterBuilder(), mapTileWriter,
				useRealTileSize);
	}
	 ...
	 *            specified by the map's {@link TileImageParameters}.
	 */
	public MapTileBuilder(AtlasCreator atlasCreator, TileImageDataWriterBuilder tileImageDataWriter,
			MapTileWriter mapTileWriter, boolean useRealTileSize) {
		this.atlasCreator = atlasCreator;
		this.tileImageDataWriterBuilder = tileImageDataWriter;
	...
	public void createTiles() throws MapCreationException, InterruptedException {
		...
		try (TileImageDataWriter tileImageDataWriter = tileImageDataWriterBuilder.build()) {
	 ...
	 * <code>xAbsPos</code> and <code>yAbsPos</code> on the currently selected map &
	 * layer.
	 *
	 * @param graphics
	 * @param xAbsPos
