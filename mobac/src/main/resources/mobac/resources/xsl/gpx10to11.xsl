...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                xmlns:gpx="http://www.topografix.com/GPX/1/0" version="1.0"
                xmlns="http://www.topografix.com/GPX/1/1" exclude-result-prefixes="gpx">

    <xsl:output method="xml" indent="yes" encoding="utf-8"/>

    <xsl:template match="/gpx:gpx">
        <gpx xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" creator="Cybarber" version="1.1"
             xsi:schemaLocation="http://www.topografix.com/GPX/1/1 http://www.topografix.com/GPX/1/1/gpx.xsd">
            <xsl:comment>xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
                xsi:schemaLocation="http://www.topografix.com/GPX/1/1 http://www.topografix.com/GPX/1/1/gpx.xsd"
            </xsl:comment>
            <xsl:comment>Transformation changes from GPX version 1.0 to version 1.1 are as follows:</xsl:comment>
            <xsl:comment>All gpx element child elements except wpt, rte and trk regrouped under newly created metadata
                element
            </xsl:comment>
            <xsl:comment>author and email re-written, author child link element and copyright elements not created.
            </xsl:comment>
            ...
            <xsl:comment>trk child elements course and speed disregarded as they are obsolete in version 1.1
            </xsl:comment>
            <xsl:comment>by William A Slabbekoorn aka Cybarber, August 2005.</xsl:comment>
            <metadata>
                <xsl:apply-templates
                        select="./*[not(local-name() = 'wpt' or local-name() = 'rte' or local-name() = 'trk')]"/>
            </metadata>
